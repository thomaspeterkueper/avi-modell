...
Das [`Residual-Space Gate v0.1`](docs/residual-space-gate-v0.1.md) findet einen logisch sauberen Residualraum, aber noch **keinen etablierten AVI-spezifischen Residualeffekt**. Ein AVI-Test darf nur mit unabhängig definierter globaler Größe, vorab festgelegten Systemsignaturen und einer gemeinsamen sparsamen Kopplung gegen `QFT+GR` antreten; bis dahin bleiben Datenfits gesperrt. Die Omnizedenz-Idee des ständigen Werdens wird dabei als Heuristik für die Frage nach erhaltener Historieninformation aufgenommen, ausdrücklich nicht als physikalische Prämisse. Nächster Schritt ist ein `Information-Retention Screen`.

Das [`Information-Retention Screen v0.1`](docs/information-retention-screen-v0.1.md) trennt vollständigen Mikrozustand, coarse graining, Irreversibilität, Hysterese, offene Quantensysteme, Korrelationen, globale Invarianten und kosmologische Anfangs-/Randinformation. Es führt `IR-0` bis `IR-3` ein. Nur `IR-3` — eine reproduzierbare Historienabhängigkeit, die weder vollständige Standard-Mikrophysik noch globale Standarddaten erklären — eröffnet einen AVI-spezifischen Retentionsraum. Bis dahin bleiben `ξ_AVI` und eine ontologische Deutung von `Φ` ausdrücklich undefiniert. Nächster Schritt ist ein `Retention Observable Screen`.

Das [`Retention Observable Screen v0.1`](docs/retention-observable-screen-v0.1.md) prüft reale Messarchitekturen. Präzisions-Frequenzverhältnisse sind der stärkste lokale Sensor, kontrollierte Hysterese/nicht-Markovsche Systeme die Negativkontrolle, QFT-Response das Nullmodell und unabhängige kosmologische Proben die globale Referenz. Daraus wird `Test R` formuliert: eine präregistrierte relationale Kopplung zwischen unabhängigem globalem `X` und mehreren lokalen dimensionslosen Responsekanälen. Ein IR-3-Diskriminator ist prinzipiell möglich, aber `X` ist noch nicht definiert; Datenfits bleiben gesperrt. Nächster Schritt: `Global-Driver Screen`.
